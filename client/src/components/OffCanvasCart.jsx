...
export default function OffCanvasCart(props) {
   ...
   const hasErrors = Object.values(errorsCreditCard).some((value) => value !== ""); // Verificar si hay algún valor en el objeto errores (algún error en los datos introducidos)
   ...
   return (
      <div>
         <Offcanvas show={show} placement={"end"} onHide={handleClose} backdrop="static" style={{ paddingTop: "70px" }}>
                  ...
                  <Accordion.Item eventKey="0">
                     ...
                     <Accordion.Body>
                        <Box style={{ display: "flex", marginBottom: "20px" }}>
                           <img className={styles.cardImage} src={creditCardImage} alt={creditCardImage} style={{ width: "40%", margin: "auto" }} />
                        </Box>
                        <Form>
                           <Form.Group className="mb-3">
                              <Form.Label>Nombre del Titular</Form.Label>
                              <Form.Control name="name" placeholder="Ingrese el nombre del titular" onChange={(e) => handleChange(e)} />
                              <p style={{ color: "red", fontSize: "12px" }}>{creditCard.touched.name ? errorsCreditCard.name : null}</p>
                           </Form.Group>
                           <Form.Group className="mb-3">
                              <Form.Label>Número de Tarjeta</Form.Label>
                              <Form.Control name="number" placeholder="Ingrese el número de la tarjeta" onChange={(e) => handleChange(e)} />
                              <p style={{ color: "red", fontSize: "12px" }}>{creditCard.touched.number ? errorsCreditCard.number : null}</p>
                           </Form.Group>
                           <Form.Group className="mb-3">
                              ...
                              <div style={{ display: "flex" }}>
                                 <DropdownButton
                                    variant="light"
                                    title={creditCard.month === "" ? "Mes" : creditCard.month}
                                    onClick={(e) => {
                                       if (e.target.name !== "") {
                                          handleChange(e);
                                       }
                              ...
                              <p style={{ color: "red", fontSize: "12px" }}>
                                 {creditCard.touched.month || creditCard.touched.year ? errorsCreditCard.month : null}
                              </p>
                           </Form.Group>

                           <Form.Group className="mb-3">
                              <Form.Label>CVV</Form.Label>
                              <Form.Control name="cvv" placeholder="Ingrese el código CVV" onChange={(e) => handleChange(e)} />
                              <p style={{ color: "red", fontSize: "12px" }}>{creditCard.touched.cvv ? errorsCreditCard.cvv : null}</p>
                           </Form.Group>
                           <Form.Group className="mb-3">
                              <Form.Label>DNI</Form.Label>
                              <Form.Control name="dni" placeholder="Ingrese el DNI del titular" onChange={(e) => handleChange(e)} />
                              <p style={{ color: "red", fontSize: "12px" }}>{creditCard.touched.dni ? errorsCreditCard.dni : null}</p>
                           </Form.Group>
                           <div
                              style={{
                                 display: "flex",
